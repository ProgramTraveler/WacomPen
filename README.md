...
* **Bug** **->** 当压力值到达像素菜单时，二级目录没有展开 **问题原因** **->** 没有将宽度比例和菜单高度相乘
...
* **另一个解决办法：** 直接在后面加制表符 **\t**就行了，直接解决这个问题，前提还是 **String** 类型
...
* 因为当时为了将数据保存为三位小数，而 **csv** 文件会自动将末尾零去掉，所以进行了一些操作，将最后的数据形式保存的是 **string** 类型，这样却不能对它进行数据分析了，所以进行一些修改
* 还是改为 **double** 类型进行记录
* 去掉三次绘制的时间记录，在实际应用中参考价值不大
